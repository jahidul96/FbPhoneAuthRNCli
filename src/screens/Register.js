...
import {AddUserToFb} from '../firebase/FbAuth';
import {addUserInfotoFb} from '../firebase/FbStore';
...
const Register = () => {
  ...
  const submitDetails = async () => {
    ...
    AddUserToFb(email, password)
      .then(data => {
        const {uid} = data.user;
        const info = {email, fullname, uid, fullphonenumber};
        addUserInfotoFb(uid, info);
      })
      .catch(err => {
      ...
      <ScrollView>
        ...
        <View style={{paddingHorizontal: 15}}>
          <View style={styles.formContainer}>
            <FormItem label="Full Name" name="person" setValue={setFullname} />
            <FormItem label="Email" name="email" setValue={setEmail} />
